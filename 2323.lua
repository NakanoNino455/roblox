...
function Library:LoadLoader()
    ...
    local SubTitle = Instance.new("TextLabel"); SubTitle.Parent = Cen; SubTitle.Text = "By CatKing"; SubTitle.Font = Enum.Font.Gotham; SubTitle.TextSize = 16; SubTitle.TextColor3 = Color3.fromRGB(220, 220, 220); SubTitle.Size = UDim2.new(1, 0, 0, 20); SubTitle.AnchorPoint = Vector2.new(1, 0); SubTitle.Position = UDim2.new(1, -20, 0.5, 25); SubTitle.TextXAlignment = Enum.TextXAlignment.Right; SubTitle.BackgroundTransparency = 1; SubTitle.TextTransparency = 1
...
local function CreateDynamicIsland()
    ...
    Island.Text="CatKing"; Island.Font=Enum.Font.GothamBold; Island.TextSize=14; Island.TextColor3=Color3.new(1,1,1); Island.AutoButtonColor=false; Island.ZIndex=100
...
function Library:SaveConfig() SaveConfig() end
...
local function CreateMobileWidget(name, toggleFunc, getEnabledState)
    ...
    Widget.InputBegan:Connect(function(i) 
        -- [Fix] 移动端悬浮窗兼容
        if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then dragging=true; dragStart=i.Position; startPos=Widget.Position end 
    end)
    Widget.InputChanged:Connect(function(i) 
        if dragging and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then local d=i.Position-dragStart; Widget.Position=UDim2.new(startPos.X.Scale,startPos.X.Offset+d.X,startPos.Y.Scale,startPos.Y.Offset+d.Y) end 
    end)
    Widget.InputEnded:Connect(function(i) 
...
function Library:CreateWindow(title, pos, isMain, isSub)
    ...
    local function RefreshH()
        ...
        if isFolded then
            ...
            -- [Fix] Explicitly set CanvasSize to Ensure scrolling works on all devices
    ...
    UserInputService.InputChanged:Connect(function(i) if drag and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then local d=i.Position-dStart; TweenService:Create(Main,TweenInfo.new(0.05),{Position=UDim2.new(sPos.X.Scale,sPos.X.Offset+d.X,sPos.Y.Scale,sPos.Y.Offset+d.Y)}):Play() end end)
    Header.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then drag=false; if (i.Position-dStart).Magnitude < 5 and tick()-sTime < 0.3 then isFolded=not isFolded; RefreshH() end end end)
    ...
    function Window:CreateModule(name, callback, allowBind)
        ...
        if allowBind then
            ...
            Btn.InputEnded:Connect(function(i) 
                if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then 
                    if ip and tick()-pt<0.6 then CreateRipple(Btn); Toggle() end; ip=false 
                end 
            end)
        ...
        function Mod:CreateSlider(txt, min, max, def, call)
            ...
            local d=false
            -- [Fix] 滑块 PC 误触修复
            T.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then d=true end end)
        ...
        function Mod:CreateDropdown(txt, opts, call)
            ...
            Main.MouseButton1Click:Connect(function() open=not open; local optionCount = #Opts:GetChildren() - 1; if optionCount < 0 then optionCount = 0 end; local nh=open and (H+(optionCount*22)) or H; local d=nh-Base.Size.Y.Offset; local nsh=SetFrame.Size.Y.Offset+d; TweenService:Create(Base,TweenInfo.new(0.3,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Size=UDim2.new(1,0,0,nh)}):Play(); TweenService:Create(SetFrame,TweenInfo.new(0.3,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Size=UDim2.new(1,0,0,nsh)}):Play(); local c; c=RunService.RenderStepped:Connect(function() List:ApplyLayout(); if winData.RefreshHeight then winData.RefreshHeight() end; if math.abs(Base.Size.Y.Offset-nh)<1 then c:Disconnect() end end) end)
            ...
            function DropController:Refresh(newOpts, resetText)
                ...
                if open then
                     local optionCount = #newOpts; local nh = H + (optionCount * 22)
                     local d=nh-Base.Size.Y.Offset; local nsh=SetFrame.Size.Y.Offset+d
                     TweenService:Create(Base,TweenInfo.new(0.3,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Size=UDim2.new(1,0,0,nh)}):Play()
                     TweenService:Create(SetFrame,TweenInfo.new(0.3,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Size=UDim2.new(1,0,0,nsh)}):Play()
                end
                if resetText then L.Text = txt.." >" end
            end
            return DropController
        ...
        -- [按钮 Button]
        function Mod:CreateButton(txt, callback)
            if not Dots.Visible then Dots.Visible=true end
            local H = 26
            local Btn = Instance.new("TextButton"); Btn.Parent = SetFrame; Btn.BackgroundTransparency = 1; Btn.Size = UDim2.new(1, 0, 0, H); Btn.Text = ""; Btn.BorderSizePixel = 0
            local L = Instance.new("TextLabel"); L.Parent = Btn; L.Text = txt; L.Font = Enum.Font.Gotham; L.TextSize = 11; L.BackgroundTransparency = 1; L.Size = UDim2.new(1, -20, 1, 0); L.Position = UDim2.new(0, 10, 0, 0); L.TextXAlignment = Enum.TextXAlignment.Left; RegisterObject(L, "TextDark")
            ...
            -- Hover effect
            Btn.MouseEnter:Connect(function() TweenService:Create(L, TweenInfo.new(0.2), {TextColor3 = CurrentThemeData.Accent}):Play() end)
            Btn.MouseLeave:Connect(function() TweenService:Create(L, TweenInfo.new(0.2), {TextColor3 = Color3.fromRGB(200,200,200)}):Play() end)
        end
